...
vector_query_tool = FunctionTool.from_defaults(name=f"vector_tool_{name}",
                                              fn=vector_query)
print("instantiating Summary Index")
summary_index = SummaryIndex(nodes)
summary_query_engine = summary_index.as_query_engine(response_mode="tree_summarize",
                                                      se_async=True,)

print("create Functiontool from SummaryIndex")
...
print("predict using summary/vector query tool")
response = llm.predict_and_call([summary_query_tool],
                                "What is India's achievement in the latest space mission ? Give details of the most Recent Project and related projects, in few paragraphs?",
                                verbose=True)
